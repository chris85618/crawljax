...
import ntut.edu.crawlguider.crawljax.plugins.domain.*;
...
public class CrawlGuiderPlugin implements OnBrowserCreatedPlugin, OnNewFoundStatePlugin, OnCountingDepthPlugin,
                                        PreStateCrawlingPlugin, PostCrawlingPlugin, OnHtmlAttributeFilteringPlugin,
                                        OnUrlLoadPlugin {
    private static final Logger LOGGER = LoggerFactory.getLogger(CrawlGuiderPlugin.class);
     ...
     */
    public CrawlGuiderPlugin(Stack<State> directivePath, ServerInstanceManagement serverInstanceManagement, int serverPort) {
    ...
    private void changeCandidateElementForCurrentState(ImmutableList<CandidateElement> candidateElements, StateVertex currentState) {
        ...
        boolean shouldCheckFormFillingResult = false;
        ...
        if (!processingDirectiveManagement.isProcessingStateHasNextActionSet()) {
            isDirectiveProcess = false;
            shouldCheckFormFillingResult = true;
            processingDirectiveManagement.removeLastStateInRecordList();
        }

        for (CandidateElement element : candidateElements) {
            if (isElementInActionSet(element.getIdentification().getValue(), actionSet)) {
                newElement = createNewCandidateElementWithFormInput(element, actionSet, shouldCheckFormFillingResult);
                break;
            }
        }
        ...
        try {
            if (newElement == null) {
                ...
                if (element != null) {
                    newElement = new CandidateElement(element, identification, "", new ArrayList<>(), actionSet.get(0).getValue(), shouldCheckFormFillingResult);
                }
                else {
                    ...
                    newElement = new CandidateElement(element, identification, "", formInputs, actionSet.get(0).getValue(), shouldCheckFormFillingResult);
                }
            }
        } catch (Exception e) {
    ...
    private CandidateElement createNewCandidateElementWithFormInput(CandidateElement element, List<Action> actions, boolean shouldCheckFormFillingResult) {
        if (actions.size() == 1 && isLinkOrButton(actions.get(0))) {
            element.setShouldCheckFormFillingResult(shouldCheckFormFillingResult);
            return element;
        }
        ...
        for (Action action : actions)
            formInputs.add(createOneFormInput(action.getValue(), action.getActionXpath()));
        return new CandidateElement(element.getElement(), element.getIdentification(), "", formInputs, "", shouldCheckFormFillingResult);
    }
